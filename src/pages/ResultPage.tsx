...
export default function ResultPage({ result, onRetry }: ResultPageProps) {
  ...
  const generateShareImage = async () => {
    ...
    ctx.textAlign = 'center';
    ctx.fillText(result.icon, W / 2, 310);
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 py-8 bg-gradient-to-b from-black to-[#2D1B69]">
      ...
      >
        ...
        >
          ...
          <p className="text-yellow-400/70 text-sm mb-1">你的职场人格是</p>
        ...
        <motion.div
          ...
          className="bg-zinc-900/80 border border-yellow-400/30 rounded-2xl p-5 mb-4"
        >
          <h3 className="text-yellow-400 text-sm font-bold mb-3">审判庭综述</h3>
        ...
        >
          <h3 className="text-yellow-400 text-sm font-bold mb-3">反思建议</h3>
        ...
        <motion.p
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 1 }}
          className="text-center text-yellow-400/50 text-xs mt-4 pb-4"
        >
          测试结果仅供娱乐，切勿当真
        </motion.p>
      </motion.div>
